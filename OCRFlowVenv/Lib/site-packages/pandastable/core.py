...
class Table(Canvas):
    ...
    def resized(self, event):
        ...
        if (
            self.currwidth != self.parentframe.winfo_width()
            or self.currheight != self.parentframe.winfo_height()
        ):
            self.redrawVisible()
        self.currwidth = self.parentframe.winfo_width()
        self.currheight = self.parentframe.winfo_height()
    ...
    def adjustColumnWidths(self, limit=30):
        ...
        for col in range(self.cols):
            ...
            tw, tl = util.getTextLength(txt, self.maxcellwidth, font=self.thefont)
            # print (col,txt,l,tw)
    ...
    def resetIndex(self, ask=True, drop=False):
        ...
        if (df.index.name is None or df.index.names[0] is None) and ask == True:
            drop = messagebox.askyesno(
                "Reset Index", "Drop the index?", parent=self.parentframe
            )
        ...
        if newname != None:
            if newname in self.model.df.columns:
                messagebox.showwarning(
                    "Name exists", "Name already exists!", parent=self.parentframe
                )
            else:
                self.storeCurrent()
    ...
    def deleteRow(self, ask=False):
        ...
        if ask == True:
            n = messagebox.askyesno(
                "Delete", "Delete selected rows?", parent=self.parentframe
            )
    ...
    def deleteColumn(self, ask=True):
        ...
        if ask == True:
            n = messagebox.askyesno(
                "Delete", "Delete Column(s)?", parent=self.parentframe
            )
        if not n:
            return
        self.storeCurrent()
    ...
    def copyColumn(self):
        ...
        new = simpledialog.askstring(
            "New name", "New name:", initialvalue=name + "1", parent=self.parentframe
        )
        if new is None:
            return
        df[new] = df[name]
    ...
    def deleteCells(self, rows, cols, answer=None):
        ...
        if answer == None:
            answer = messagebox.askyesno(
                "Clear Confirm", "Clear this data?", parent=self.parentframe
            )
        if not answer:
            return
        self.storeCurrent()
    ...
    def autoAddColumns(self, numcols=None):
        ...
        if numcols == None:
            numcols = simpledialog.askinteger(
                "Auto add rows.", "How many empty columns?", parent=self.parentframe
            )
    ...
    def applyColumnFunction(self, evt=None):
        ...
        d = MultipleValDialog(
            ...
            labels=("Function:", "New column name:", "In place:", "New column suffix:"),
    ...
    def valueCounts(self):
        ...
        if len(cols) < 2:
            ...
            df = df.pivot_table(
                index=cols[0], columns=cols[1:], aggfunc="size", fill_value=0
            ).T
    ...
    def convertDates(self):
        ...
        if len(cols) == 1 and temp.dtype == "datetime64[ns]":
            ...
            try:
                ...
                logging.error("Exception occurred", exc_info=True)
                messagebox.showwarning("Convert error", e, parent=self.parentframe)
                return
    ...
    def functionsBar(self, evt=None):
        ...
        f = Combobox(ef, values=funcs, textvariable=self.funcvar)
    ...
    def evalBar(self, evt=None):
        ...
        self.estyle = Style()
        self.estyle.configure("White.TCombobox", fieldbackground="white")
        self.estyle.configure("Red.TCombobox", fieldbackground="#ffcccc")
        ...
        self.evalvar = StringVar()
        funclist = ["=".join(i) for i in self.formulae.items()]
        self.functionentry = e = Combobox(
            bf,
            values=funclist,
            textvariable=self.evalvar,
            width=34,
        ...
        addButton(
            bf, "clear", clear, images.delete(), "clear stored functions", side=LEFT
        )
        ...
        Checkbutton(bf, text="Place new columns", variable=self.placecolvar).pack(
            side=LEFT
        )
        Checkbutton(bf, text="Recalculate all", variable=self.recalculatevar).pack(
            side=LEFT
        )
        return
    ...
    def isInsideTable(self, x, y):
        ...
        if (
            self.x_start < x < self.tablewidth
    ...
    def handle_right_click(self, event):
        ...
        if (
            rowclicked in self.multiplerowlist or self.allrows == True
        ) and colclicked in self.multiplecollist:
    ...
    def pasteTable(self, event=None):
        ...
        try:
            ...
            messagebox.showwarning("Could not read data", e, parent=self.parentframe)
            return
        if len(df) == 0:
            return
    ...
    def convertNumeric(self):
        ...
        for c in colnames:
            ...
            if fillempty == 1:
                x = x.fillna(0)
            if currency == 1:
                x = x.replace("[\$\£\€,)]", "", regex=True).replace(
                    "[(]", "-", regex=True
                )
            if removetext == 1:
    ...
    def popupMenu(self, event, rows=None, cols=None, outside=None):
        ...
        main = ["Copy", "Undo", "Fill Down", "Clear Data", "Set Color"]  # "Fill Right",
        general = [
            "Select All",
            "Filter Rows",
            "Show as Text",
    ...
    def loadExcel(self, filename=None):
        ...
        if filename == None:
            filename = filedialog.askopenfilename(
                ...
                initialdir=os.getcwd(),
                filetypes=[("xls", "*.xls"), ("xlsx", "*.xlsx"), ("All files", "*.*")],
            )
        if not filename:
            return
...
class ToolBar(Frame):
    ...
    def __init__(self, parent=None, parentapp=None):
        ...
        img = images.copy()
        addButton(
            self, "Copy", self.parentapp.copyTable, img, "copy table to clipboard"
        )
        img = images.paste()
        ...
        img = images.fit()
        addButton(
            self, "Stats models", self.parentapp.statsViewer, img, "model fitting"
        )
...
class statusBar(Frame):
    ...
    def __init__(self, parent=None, parentapp=None):
        ...
        img = images.zoom_in()
        addButton(
            fr, "Zoom In", self.parentapp.zoomIn, img, "zoom in", side=LEFT, padding=1
        )
        return
